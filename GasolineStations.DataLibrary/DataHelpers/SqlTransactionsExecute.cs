...
namespace GasolineStations.DataLibrary.DataHelpers
{
    public class SqlTransactionsExecute
    {
                    ...
                    foreach (PlacePrices ItemPlace in placePrice.ToList())
                    {
                        ...
                        if (Result.FuelStationId > 0 && _ResultPrice.Count == 0)
                        {
                            ...
                            foreach (var itemPrice in ItemPlace.DetailGasPriceType)
                            {
                                ...
                                command.Parameters.Add("@Gas_PriceType", SqlDbType.VarChar, 50).Value = itemPrice.Type;
        ...
        private List<TblFuelStationModel> GetFuelStations()
        {
        ...
        private List<TblFuelPriceModel> GetFuelStPrices(string FuelStationId)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand($"{" pp.FuelStationId, pp.Gas_Price, pp.Placegas_PriceId,pp.Gas_PriceType "}{" "}{" from [ProductServices].[Placegas_Prices] pp With(Nolock) Where pp.FuelStationId =@FuelStationId "}", con))
                {
                    cmd.CommandType = CommandType.Text;
                    con.Open();
                ...
                try
                {
                    ...
                    foreach (Places Items in places.ToList())
                    {
                        ...
                        if (Result.FuelStationId > 0)
                        {
                            ...
                            command.Parameters.Add("@PermitNumber", SqlDbType.VarChar).Value = Items.cre_id;
                            command.Parameters.Add("@FuelName", SqlDbType.VarChar).Value = Items.PlaceName;
